...
st.markdown(""" # Search for Public Water Systems
  ...
  ### Click on the map to retrieve information about nearby public water systems
...
def main():
  with st.spinner(text="Loading map..."):
    ...
    c1, c2 = st.columns(2)
    with c1:
      output = st_folium(m, width=700, height=500, feature_group_to_add=fg, returned_objects=["last_clicked"]) #

    #with c2:
      #st.write(output)

    if ((output["last_clicked"]) and (output["last_clicked"] != st.session_state["last_clicked"])):
      st.session_state["last_clicked"] = output["last_clicked"]
      # Get and map data
      x = output["last_clicked"]["lat"]
      y = output["last_clicked"]["lng"]
      ...
      # Get data
      #data = get_data_from_ids("SDWA_PUBLIC_WATER_SYSTEMS_MVIEW", "PWSID", these_pws)
